...
data "template_file" "provision_k3s_cluster2_server" {
  template = var.k8s_distro_name == "k3s" ? file("${path.module}/user-data-scripts/provision_k3s_server.sh.tpl") : null
  vars = {
    k3s_cluster2_cluster_secret = random_password.cluster2_secret.result
...
# Generate template file for k3s cluster2 agent
...
data "template_file" "provision_rke2_server" {
  ...
  vars = {
    ...
    rke2_server_public_ip = aws_eip.aws_eip_controlplane[0].public_ip
    rke2_version =  local.k8s_distro_version
    selinux_mode = var.selinux_mode
  }
}
...
data "template_file" "provision_rke2_agent" {
  ...
  vars = {
    rke2_server_url = "https://${aws_eip.aws_eip_controlplane[0].public_ip}:9345"
    rke2_cluster_secret = random_password.cluster_secret.result
    rke2_version =  local.k8s_distro_version
    selinux_mode = var.selinux_mode
  }
...
# Generate template file for rke2 cluster2 server
data "template_file" "provision_rke2_cluster2_server" {
  template = var.k8s_distro_name == "rke2" ? file("${path.module}/user-data-scripts/provision_rke2_server.sh.tpl") : null
  vars = {
    rke2_cluster_secret = random_password.cluster2_secret.result
    rke2_server_public_ip = aws_eip.aws_eip_cluster2_controlplane[0].public_ip
    rke2_version =  local.k8s_distro_version
    selinux_mode = var.selinux_mode
  }
}

# Generate template file for rke2 cluster2 agent
data "template_file" "provision_rke2_cluster2_agent" {
  template = var.k8s_distro_name == "rke2" ? file("${path.module}/user-data-scripts/provision_rke2_agent.sh.tpl") : null
  vars = {
    rke2_server_url = "https://${aws_eip.aws_eip_cluster2_controlplane[0].public_ip}:9345"
    rke2_cluster_secret = random_password.cluster2_secret.result
    rke2_version =  local.k8s_distro_version
    selinux_mode = var.selinux_mode
  }
