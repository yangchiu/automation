...
resource "aws_instance" "aws_instance_controlplane" {
 depends_on = [
  ...
  ami           = data.aws_ami.aws_ami_version.id
  ...
  user_data = var.k8s_distro_name == "k3s" ? data.template_file.provision_k3s_server.rendered : (var.k8s_distro_name == "rke2" ? data.template_file.provision_rke2_server.rendered : data.template_file.provision_rke.rendered)
...
resource "aws_instance" "aws_instance_worker" {
  ...
  user_data = var.k8s_distro_name == "k3s" ? data.template_file.provision_k3s_agent.rendered : (var.k8s_distro_name == "rke2" ? data.template_file.provision_rke2_agent.rendered : data.template_file.provision_rke.rendered)
...
resource "null_resource" "rsync_kubeconfig_file" {
  ...
  provisioner "local-exec" {
    command = var.k8s_distro_name == "k3s" ? "rsync -aPvz --rsync-path=\"sudo rsync\" -e \"ssh -o StrictHostKeyChecking=no -l ec2-user -i ${var.aws_ssh_private_key_file_path}\" ${aws_eip.aws_eip_controlplane[0].public_ip}:/etc/rancher/k3s/k3s.yaml .  && sed -i 's#https://127.0.0.1:6443#https://${aws_eip.aws_eip_controlplane[0].public_ip}:6443#' k3s.yaml"  : "echo \"rke ... skipping\""
  }
}
...
resource "null_resource" "rsync_kubeconfig_file_rke2" {
  ...
  provisioner "local-exec" {
    command = var.k8s_distro_name == "rke2" ? "rsync -aPvz --rsync-path=\"sudo rsync\" -e \"ssh -o StrictHostKeyChecking=no -l ec2-user -i ${var.aws_ssh_private_key_file_path}\" ${aws_eip.aws_eip_controlplane[0].public_ip}:/etc/rancher/rke2/rke2.yaml .  && sed -i 's#https://127.0.0.1:6443#https://${aws_eip.aws_eip_controlplane[0].public_ip}:6443#' rke2.yaml" : "echo \"not rke2 ... skipping\""
  }
}
...
resource "aws_instance" "aws_instance_cluster2_controlplane" {
 depends_on = [
  ...
  key_name = aws_key_pair.aws_pair_key.key_name
  user_data = var.k8s_distro_name == "k3s" ? data.template_file.provision_k3s_cluster2_server.rendered : (var.k8s_distro_name == "rke2" ? data.template_file.provision_rke2_cluster2_server.rendered : data.template_file.provision_rke.rendered)
...
resource "aws_instance" "aws_instance_cluster2_worker" {
  ...
  user_data = var.k8s_distro_name == "k3s" ? data.template_file.provision_k3s_cluster2_agent.rendered : (var.k8s_distro_name == "rke2" ? data.template_file.provision_rke2_cluster2_agent.rendered : data.template_file.provision_rke.rendered)
...
resource "null_resource" "rsync_kubeconfig_file_cluster2" {
  ...
  provisioner "local-exec" {
    command = var.k8s_distro_name == "k3s" ? "rsync -aPvz --rsync-path=\"sudo rsync\" -e \"ssh -o StrictHostKeyChecking=no -l ec2-user -i ${var.aws_ssh_private_key_file_path}\" ${aws_eip.aws_eip_cluster2_controlplane[0].public_ip}:/etc/rancher/k3s/k3s.yaml k3s_cluster2.yaml  && sed -i 's#https://127.0.0.1:6443#https://${aws_eip.aws_eip_cluster2_controlplane[0].public_ip}:6443#' k3s_cluster2.yaml"  : "echo \"rke ... skipping\""
  }
}
...
resource "null_resource" "rsync_kubeconfig_file_rke2_cluster2" {
  ...
  provisioner "local-exec" {
    command = var.k8s_distro_name == "rke2" ? "rsync -aPvz --rsync-path=\"sudo rsync\" -e \"ssh -o StrictHostKeyChecking=no -l ec2-user -i ${var.aws_ssh_private_key_file_path}\" ${aws_eip.aws_eip_cluster2_controlplane[0].public_ip}:/etc/rancher/rke2/rke2.yaml rke2_cluster2.yaml  && sed -i 's#https://127.0.0.1:6443#https://${aws_eip.aws_eip_cluster2_controlplane[0].public_ip}:6443#' rke2_cluster2.yaml" : "echo \"not rke2 ... skipping\""
  }
}
